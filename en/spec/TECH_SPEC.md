...
### 3.4 Route Meta & Navigation Guards
...
```javascript
// lib/api/cart.js — one file per resource
...
// Individual components only need to handle business logic errors (show message).
```

---

## 8. Pinia Store Conventions
...
// ✅ Define validationSchema inside setup or in a separate file for reuse
const { handleSubmit, errors } = useForm({
  validationSchema: {
    name:  (val) => val?.length >= 2 || 'Name must be at least 2 characters',
    phone: (val) => /^09\d{8}$/.test(val) || 'Please enter a valid phone number',
    email: (val) => !val || /.+@.+\..+/.test(val) || 'Please enter a valid email',
  },
})
...
<template>
  <!-- Pair with Vuetify — pass errors directly to error-messages -->
  <v-text-field
    v-model="name"
    label="Name"
...
## 12. Toast / Notification Conventions
...
## 13. Default Responsive Breakpoints
